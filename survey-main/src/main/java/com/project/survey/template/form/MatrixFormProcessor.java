...
public class MatrixFormProcessor extends AbstractFormProcessor {
	...
	protected void internalProcess(Node content, Map<String, Object> parameters) throws FormProcessorFailedException {
		...
		try {
			...
			if(node.hasNode("matrix")) {
				matrix = node.getNode("matrix");
			}
			else {
				matrix = NodeUtil.createPath(node, "matrix", "mgnl:content");
			}

...
public class ChallengesFormProcessor extends AbstractFormProcessor {
		...
		protected void internalProcess(Node content, Map<String, Object> parameters) throws FormProcessorFailedException {
			...
			try {
				...
				if(node.hasNode("challeges")) {
					matrix = node.getNode("challeges");
				}
				else {
					matrix = NodeUtil.createPath(node, "challeges", "mgnl:content");
				}

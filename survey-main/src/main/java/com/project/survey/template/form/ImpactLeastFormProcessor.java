...
public class ImpactLeastFormProcessor extends AbstractFormProcessor {
		...
		protected void internalProcess(Node content, Map<String, Object> parameters) throws FormProcessorFailedException {
			...
			try {
				...
				if(node.hasNode("impact-least")) {
					matrix = node.getNode("impact-least");
				}
				else {
					matrix = NodeUtil.createPath(node, "impact-least", "mgnl:content");
				}

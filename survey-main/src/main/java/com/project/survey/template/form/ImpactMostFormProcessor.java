...
public class ImpactMostFormProcessor extends AbstractFormProcessor {
		...
		protected void internalProcess(Node content, Map<String, Object> parameters) throws FormProcessorFailedException {
			...
			try {
				...
				if(node.hasNode("impact-most")) {
					matrix = node.getNode("impact-most");
				}
				else {
					matrix = NodeUtil.createPath(node, "impact-most", "mgnl:content");
				}
